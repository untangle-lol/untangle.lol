...
const [lang,setLang]=useState(null);
  ...
  // Sync view → URL + document.title (single source of truth — no second effect needed)
  useEffect(()=>{
    ...
    const s=' — untangle.lol';
    const titles={
      home:t.pageTitle||('untangle.lol'+s),
      woop_input:(t.woopTitle||'WOOP')+s,
      no_credits:(t.payTitle||'Get more questions')+s,
      donate:(t.donate||'Donate')+s,
      terms:(t.terms||'Terms')+s,
      privacy:(t.privacy||'Privacy')+s,
      feedback:(t.fbTitle||'Feedback')+s,
    };
    document.title=titles[vw]||(t.pageTitle||('untangle.lol'+s));
  },[vw,ready,t]);// eslint-disable-line react-hooks/exhaustive-deps
...
const langSv=ls.get("untangle_lang");if(langSv)setLang(langSv);
    ...
    try{
      ...
      if(sd.user?.email){
        ...
        setAuth("in");
        // Load account credits from server (authoritative source)
        try{const cr=await fetch("/api/me/credits");const cd=await cr.json();if(typeof cd.balance==="number")setCredits(cd.balance);}
        catch{setCredits(FREE_CREDITS);}
        let parsedHist=[];
  ...
  const pollCredits=async(ref,attempts=0,provider="unknown",tier="unknown",creditsExpected=0)=>{
    ...
    try{
      const fp=fpRef.current?`&fp=${encodeURIComponent(fpRef.current)}`:"";
      const r=await fetch(`/api/credits/verify?ref=${encodeURIComponent(ref)}${fp}`);
      const d=await r.json();
      if(d.credits>0){
        // Auth users: server returned authoritative balance; guests: add delta
        if(userRef.current&&typeof d.balance==="number"){setCredits(d.balance);}
        else{addCredits(d.credits);}
  ...
  const addCredits=(n)=>{
    if(userRef.current){
      // Auth users: server is authoritative; optimistic UI update only
      setCredits(prev=>prev+n);
    }else{
  ...
  const sendMagicLink=async(e)=>{
    ...
    try{
      const r=await fetch("/api/auth/magic/request",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,lang:lang||"en"})});
  ...
  const deductCredit=()=>{
    if(userRef.current){
      // Auth users: server already deducted; reflect server response in state
      // (creditsRemaining from chat response is applied at the call site)
    }else{
      const key=activeCreditsKey();
  ...
  const buildPool=(tLang,gSugg)=>{
    ...
    for(let i=0;pool.length<total;i++){
      if(ai<alt.length&&(i%4===3||ni>=normal.length)){pool.push({text:alt[ai++],kind:"alt"});}
      else if(ni<normal.length){pool.push({text:normal[ni++],kind:"normal"});}
      else break;
    }
    return pool;
  };
  ...
  const RecentsSection=()=>{
    ...
    return(
      <div style={{marginTop:10,border:"1px solid "+c.cb,borderRadius:10,overflow:"hidden"}}>
        <div style={{background:c.sb,borderBottom:"1px solid "+c.cb,padding:"8px 12px"}}>
          <span style={{fontSize:13,fontWeight:600,color:c.tf,display:"inline-flex",alignItems:"center",gap:5}}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{display:"block",flexShrink:0}}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
            {t.recentQ||"Recent questions"}
          </span>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:0}}>
          {recents.map((r,i)=>(
            <div key={"l"+i} style={{...chipRow,borderTop:i>0?"1px solid "+c.cb:"none"}}>
              <button onClick={()=>pick(r)} style={{flex:1,background:"none",border:"none",padding:"9px 12px",color:c.tm,cursor:"pointer",fontSize:13,textAlign:"left",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r}</button>
              {delBtn(()=>delRecent(r))}
            </div>
          ))}
        </div>
  ...
  const claimAltruismBonus=()=>{
    ...
    if(bonus>0){
      if(auth==="in"){
        fetch("/api/me/credits",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({delta:bonus,reason:"altruism"})})
          .then(r=>r.ok?r.json():null).then(d=>{if(typeof d?.balance==="number")setCredits(d.balance);}).catch(()=>{});
      }
      addCredits(bonus);
        ...
        <div style={{position:"fixed",top:"calc(48px + env(safe-area-inset-top))",right:0,width:"min(280px,85vw)",zIndex:199,background:nbBg,backdropFilter:"blur(16px)",borderLeft:"1px solid "+c.cb,borderBottom:"1px solid "+c.cb,borderBottomLeftRadius:16,boxShadow:rt==="dark"?"0 16px 48px rgba(0,0,0,0.5)":"0 16px 48px rgba(0,0,0,0.12)",overflow:"hidden",animation:"slideInRight 0.2s cubic-bezier(0.16,1,0.3,1)"}}>
          ...
          {showCred&&(
            <Row
              ...
              label={<span>{credits} <span style={{color:c.tf,fontWeight:400}}>{t.cred||"questions"} {t.remaining||"remaining"}</span></span>}
  ...
  return(<>
    ...
    {vw==="no_credits"&&(()=>{
      ...
      if(auth!=="in") return (
        <div dir={dir} style={sx.pg}><div className="uw" style={sx.w}>
          <div style={{...sx.cd,padding:"28px 24px 24px",display:"flex",flexDirection:"column",alignItems:"center",gap:14}}>
            <div style={{width:48,height:48,borderRadius:14,background:c.ab,border:"1px solid "+c.abr,display:"flex",alignItems:"center",justifyContent:"center",color:c.ac,flexShrink:0}}>
            ...
            <div style={{textAlign:"center"}}>
              <h2 style={{fontSize:18,fontWeight:700,color:c.tx,margin:"0 0 4px"}}>{t.payLoginRequired||"Sign in to buy questions"}</h2>
              <p style={{fontSize:13,color:c.tm,lineHeight:1.5,margin:0}}>{t.payLoginDesc||"Create a free account to purchase questions and keep them across devices."}</p>
            </div>
            <a href="/api/auth/google" style={{display:"inline-flex",alignItems:"center",gap:10,padding:"10px 20px",background:c.card,border:"1px solid "+c.cb,borderRadius:10,fontSize:13,fontWeight:600,color:c.tx,textDecoration:"none",width:"100%",justifyContent:"center",boxSizing:"border-box"}}>
            ...
            {magicState==="sent"?(
              ...
              <form onSubmit={sendMagicLink} style={{display:"flex",gap:8,width:"100%"}}>
                <input type="email" value={magicEmail} onChange={e=>setMagicEmail(e.target.value)} placeholder={t.magicPh||"your@email.com"} disabled={magicState==="loading"} style={{flex:1,padding:"9px 12px",borderRadius:8,border:"1px solid "+(magicState==="error"||magicState==="ratelimit"?"#ef4444":c.cb),background:c.card,color:c.tx,fontSize:13,fontFamily:"inherit",outline:"none",minWidth:0}}/>
                <button type="submit" disabled={magicState==="loading"||!magicEmail.trim()} style={{flexShrink:0,padding:"9px 14px",borderRadius:8,background:c.ac,color:"#fff",border:"none",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",opacity:(magicState==="loading"||!magicEmail.trim())?0.6:1}}>{magicState==="loading"?"…":(t.magicBtn||"Send link")}</button>
              </form>
            )}
            {(magicState==="error"||magicState==="ratelimit")&&(
              <p style={{margin:"-6px 0 0",fontSize:11,color:"#ef4444",alignSelf:"flex-start"}}>{magicState==="ratelimit"?(t.magicRateLimit||"Too many attempts. Try again later."):(t.magicError||"Something went wrong. Try again.")}</p>
            )}
            <button onClick={()=>setVw("home")} style={{...sx.bg,marginTop:0,width:"100%"}}>{t.back}</button>
          </div>
        <BottomBar/><style>{GS}</style></div></div>
      );
      return (
        ...
        <div dir={dir} style={sx.pg}><div className="uw" style={sx.w}>
          ...
          <div style={{display:"flex",gap:4,marginBottom:20,alignItems:"stretch"}}>
            {[0,1,2,3].map(i=>{
              ...
              return(
                <button key={i} onClick={()=>setWoopStep(i)} style={{flex:active?2:1,height:36,borderRadius:8,border:"1px solid "+(active?c.abr:done?c.abr:c.cb),background:active?c.ab:done?"transparent":"transparent",cursor:"pointer",padding:"0 10px",display:"flex",alignItems:"center",justifyContent:"center",gap:5,transition:"all 0.2s ease",overflow:"hidden",minWidth:0}}>
                  <span style={{fontSize:done||active?11:11,fontWeight:700,color:active?c.ac:done?c.ac:c.tf,letterSpacing:"0.04em",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{active?labels[i].toUpperCase():done?"✓":(i+1)}</span>
                </button>
              );
            })}
          </div>
          ...
          {woopStep>0&&(
            <div style={{marginTop:10,display:"flex",flexDirection:"column",gap:5}}>
              {woopKeys.slice(0,woopStep).map((k,i)=>woopData[k]&&(
                <div key={k} onClick={()=>setWoopStep(i)} style={{padding:"8px 12px",borderRadius:8,background:c.ab,border:"1px solid "+c.abr,display:"flex",gap:8,alignItems:"flex-start",cursor:"pointer"}}>
                  <span style={{color:c.ac,flexShrink:0,marginTop:1}}>{WOOP_ICONS[i]}</span>
                  <div style={{minWidth:0}}>
                    <div style={{fontSize:10,fontWeight:700,color:c.ac,marginBottom:1,letterSpacing:"0.05em"}}>{labels[i].toUpperCase()}</div>
