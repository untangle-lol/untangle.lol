...
export default function App(){
  ...
  const SuggChips=()=>{
    ...
    // Interleave altruistic suggestions: every other chip is altruistic (💛), rest are regular
    const altPool=(t.altruisticSugg||[]).filter(s=>!localSet.has(s.toLowerCase()));
    const interleaved=[];
    let gi=0,ai=0;
    while(gi<globals.length||ai<altPool.length){
      if(gi<globals.length)interleaved.push({text:globals[gi++],kind:"global"});
      if(ai<altPool.length)interleaved.push({text:altPool[ai++],kind:"alt"});
    }
